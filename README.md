...
2. 購入済みライブラリの通常の「ダウンロード」リンクから、商品・バリエーション・ファイルのIDを検査します。
...
- 商品ID・バリエーションID・ファイルIDを一意に確認できた組み合わせでだけ「その他のDL方法」を隠し、通常ダウンロードをアプリの保存処理へ接続します。ページ構造が想定と異なる場合は代替操作を残して安全側に停止します。
