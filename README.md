...
- サイドバーの展開状態にかかわらず「BOOTH非公式アプリ」を常時表示
- ローカル固定ドライブを通常の保存先とし、ネットワーク・同期・判定不能な保存先は警告と明示同意を要求

公式BOOTH Library Managerのデータベース、保存先、Windowsの`booth-library-manager://`プロトコル登録は参照・変更しません。

## 動作の概要

1. 専用WebViewでBOOTHへ直接ログインします。認証情報をStashly for BOOTHのコードへ入力する方式ではありません。
...
6. 商品名とショップ名は、該当する公開商品ページのOpen Graphメタデータから取得します。成功結果は30日間キャッシュし、取得失敗時は24時間再試行せず、BOOTHから429応答を受けた場合は全商品について最低24時間停止します。
...
GitHub Releasesから`Stashly for BOOTH_*_x64-setup.exe`を取得して実行します。
...
1. 設定画面で、Stashly for BOOTH専用の保存先を選択します。公式BOOTH Library Managerの保存先とは分けてください。通常はローカル固定ドライブを使用してください。
...
- ネットワーク、同期、または種類を確認できない保存先を選ぶと、保存先ごとの警告と明示同意が必要です。同意は別の保存先へ流用されず、ダウンロード前にも保存先の到達性と同意を再確認します。
...
- ダウンロードした商品とZIP内ファイルは信頼せず、ウイルス対策ソフト等で確認してください。Stashly for BOOTHが展開したファイルを自動実行することはありません。
- 設定画面の一括削除は、Stashly for BOOTHのSQLiteに登録されたファイルと展開フォルダを完全に削除します。ごみ箱には移動せず、元に戻せません。専用の保存先を使用し、確認画面の対象パスを必ず確認してください。
- UNC、NAS、マップドライブ、同期フォルダーでは、切断、遅延、同期競合、プレースホルダーによって処理が失敗する場合があります。同期はバックアップではなく、保存先が購入者本人以外へ共有されないことを確認してください。すべての同期ソフトや仮想ドライブを自動判定できるわけではありません。
