...
- 商品カードの右クリックメニューから、保存フォルダを開くか商品情報を個別に再取得
- 最大2ファイルを同時にダウンロードし、同じ商品の重複ダウンロードを防止
- Windowsの設定に連動するテーマ、ライトテーマ、ダークテーマとアクセントカラーを選択
- 起動時または設定画面から新しいバージョンを確認し、アプリ内で署名検証・更新・再起動
...
Updaterを初めて搭載するバージョンへ移行するときだけは、従来どおりGitHub Releasesからインストーラを取得して手動で更新する必要があります。そのバージョン以降は、起動時に新しい公開済みReleaseを確認し、確認画面の「更新して再起動」からアプリ内で更新できます。更新パッケージの署名検証は、WindowsのAuthenticode署名とは別の仕組みです。
...
4. **ローカルで使う** — ダウンロードが終わると「ローカルライブラリ」に商品が追加されます。商品カードから保存フォルダを開けます。カードを右クリックすると、商品情報だけを再取得することもできます。
5. **Stashlyを更新する** — 新しい公開済みバージョンがあると確認画面が表示されます。「更新して再起動」を選ぶと、更新パッケージを取得して署名を確認し、更新後にStashlyを再起動します。商品をダウンロード中の場合は、完了後にもう一度実行してください。設定画面から手動確認もできます。
...
- 本ポリシーへの同意後、起動時にGitHub Releasesへ最新版を確認します。更新パッケージは利用者が実行を選んだ場合だけ取得します。
...
- ダウンロード済みの商品ファイルを上書きする機能、タグ編集、ZIP以外の自動展開には対応していません。
